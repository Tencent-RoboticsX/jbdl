...
class MountainCar(BaseEnv):
    def __init__(self, pure_mountain_car_params=DEFAULT_PURE_MOUNTAIN_CAR_PARAMS, reward_fun=None, seed=123, sim_dt=math.sqrt(0.0025/9.81), batch_size=0, render=False, screen_width=600, screen_height=400, render_idx=None):
        ...
        super().__init__(pure_mountain_car_params, seed=seed, sim_dt=sim_dt, batch_size=batch_size, render=render, render_engine=rendering.Viewer(screen_width, screen_height), render_idx=render_idx)

        def _dynamics_fun_core(pos_vel, action,  m_car, f_unit, a_gravity):
            position = pos_vel[0:1]
            velocity = pos_vel[1:2]
            acceleration = (action - 1) * f_unit / m_car + jnp.cos(3 * position) * a_gravity
            vel_acc = jnp.hstack([velocity, acceleration])
            return vel_acc

        self._dynamics_fun = partial(_dynamics_fun_core, a_gravity=self.a_gravity)
        ...
        def _dynamics_step_core(y0, *dynamics_args, min_position, max_position, max_delta_position, sim_dt):
            ...
            velocity = jnp.clip(velocity, -max_delta_position/sim_dt, max_delta_position/sim_dt)
            position += velocity * sim_dt
            position = jnp.clip(position, min_position, max_position)


            velocity = jax.lax.cond(
                jnp.all(jnp.hstack([jnp.heaviside(min_position-position, 1.0), jnp.heaviside(-velocity, 0.0)])),
    ...
    def _state_random_initializer(self):
        ...
        state = np.array([jax.random.uniform(subkey, minval=-0.6, maxval=-0.4), 0.0])
        return state

    def _batch_state_random_initializer(self, idx_list):
        if idx_list is None:
            self.key, subkey = jax.random.split(self.key)
            state = jnp.concatenate([jax.random.uniform(subkey, shape=(self.batch_size, 1), minval=-0.6, maxval=-0.4), jnp.zeros((self.batch_size, 1))], axis=1)
            return state
        else:
            idx_num = len(idx_list)
            self.key, subkey = jax.random.split(self.key)
            update_state = jnp.concatenate([jax.random.uniform(subkey, shape=(idx_num, 1), minval=-0.6, maxval=-0.4), jnp.zeros((idx_num, 1))], axis=1)
            state = index_update(
                self.state,
                index[idx_list, :],
                update_state
    ...
    def _batch_step_fun(self, action):
        ...
        dynamics_params = (action, self.m_car, self.f_unit)
        next_state = jax.vmap(self.dynamics_step, (0, 0, None, None), 0)(self.state, *dynamics_params)
