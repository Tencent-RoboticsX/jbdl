...
from jbdl.envs.utils.urdf_utils import (
    parse_origin,
    unparse_origin,
    get_filename,
    load_meshes,
...
class Geometry(URDFType):
    ...
    def copy(self, prefix='', scale=None):
        ...
        v = Geometry(
            box=(
                self.box.copy(
                    prefix=prefix, scale=scale) if self.box else None), cylinder=(
                self.cylinder.copy(
                    prefix=prefix, scale=scale) if self.cylinder else None), sphere=(
                        self.sphere.copy(
                            prefix=prefix, scale=scale) if self.sphere else None), mesh=(
                                self.mesh.copy(
                                    prefix=prefix, scale=scale) if self.mesh else None), )
        return v
    ...
    def copy(self, prefix='', scale=None):
        ...
        return Visual(
            geometry=self.geometry.copy(
                prefix=prefix, scale=scale), name='{}{}'.format(
                prefix, self.name), origin=origin, material=(
                self.material.copy(
                    prefix=prefix) if self.material else None), )
    ...
    def get_child_poses(self, cfg, n_cfgs):
        ...
        if cfg is None:
            ...
            if cfg is None:
                cfg = np.zeros(n_cfgs)
            return np.matmul(
                self.origin, self._rotation_matrices(
                    cfg, self.axis))
        elif self.joint_type == 'prismatic':
            if cfg is None:
                cfg = np.zeros(n_cfgs)
...
class Link(URDFType):
    ...
    def copy(self, prefix='', scale=None, collision_only=False):
        ...
        if not collision_only:
            visuals = [v.copy(prefix=prefix, scale=scale)
        ...
        cpy = Link(
            ...
            collisions=[
                v.copy(
                    prefix=prefix,
                    scale=scale) for v in self.collisions],
        )
...
class URDF(URDFType):
    ...
    def link_fk_batch(self, cfgs=None, link=None, links=None, use_names=False):
        ...
        # Compute FK mapping each link to a vector of matrices, one matrix per
        # cfg
        fk = OrderedDict()
        for lnk in self._reverse_topo:
            if lnk not in link_set:
