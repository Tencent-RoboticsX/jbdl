...
def CalcPointJacobianDerivativeCore(Xtree, parent, jtype, jaxis, body_id, NB, q, qdot, point_pos):
...
def CalcPointJacobianDerivative(model: dict, q: np.ndarray, qdot: np.ndarray, body_id: int, point_pos: np.ndarray)->np.ndarray:
    ...
    JDot = CalcPointJacobianDerivativeCore(Xtree, tuple(parent), tuple(jtype), jaxis, body_id, NB, q, qdot, point_pos)
    return JDot
